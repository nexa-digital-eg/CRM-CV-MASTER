...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 min-h-screen flex flex-col" style={{ background: 'linear-gradient(180deg,#0f172a 0%,#1a1040 100%)' }}>
      {/* Logo */}
      <div className="px-6 py-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-bold text-sm leading-tight">CV Master</p>
        ...
        {navItems.map(({ key, href, icon }, i) => {
          ...
          return (
            <Link
              key={key}
              href={fullHref}
              className={`nav-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium ${
                isActive
                  ? 'active bg-indigo-600/30 text-indigo-200 sidebar-active-glow'
                  : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'
              }`}
              style={{ animationDelay: `${i * 40}ms`, animation: 'slideInLeft .35s ease both' }}
            >
